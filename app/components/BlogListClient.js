 ...
 * BlogListClient
 *
 * Receives blogs that already have a plain-text `excerpt` field generated
 * on the server (via stripMarkdown). This means we never need to import
 * the heavy react-markdown / remark / rehype stack here, keeping the
 * client bundle small.
 */
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      items = items.filter((b) => {
        const haystack = `${b.title} ${b.excerpt || ''} ${b.author || ''}`.toLowerCase()
    ...
    <div>
          ...
          <div className="md:flex">
            ...
            <div className="p-6 md:flex-1">
              <div className="text-sm text-gray-500 mb-2">
                {new Date(hero.date_posted).toLocaleDateString('en-US')} · {hero.author}
              </div>
              <h2 className="text-2xl font-bold mb-3">{hero.title}</h2>
              {hero.excerpt && (
                <p className="text-gray-700 mb-4 line-clamp-3">{hero.excerpt}</p>
              )}
              <div className="flex items-center gap-3">
                <Link
                  href={`/blog/${hero.slug}`}
      ...
      <ul className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {rest.map((b) => (
          <li
            key={b.id}
            className="bg-white shadow-sm hover:shadow-lg transform transition hover:-translate-y-1 rounded-lg overflow-hidden"
          >
            <Link href={`/blog/${b.slug}`} className="block group" aria-label={`Read ${b.title}`}>
              {b.image ? (
                ...
                <div className="h-44 md:h-48 w-full bg-gradient-to-r from-gray-100 to-gray-50" />
              )}

              <div className="p-4">
                <h3 className="text-lg font-semibold text-gray-900 mb-1">{b.title}</h3>
                <div className="text-xs text-gray-500 mb-3">
                  {new Date(b.date_posted).toLocaleDateString('en-US')} · {b.author}
                </div>
                {b.excerpt && (
                  <p className="text-sm text-gray-700 mb-4 line-clamp-3">{b.excerpt}</p>
                )}
              </div>
            </Link>

            {/* Action row outside the Link to avoid nested <a> tags */}
