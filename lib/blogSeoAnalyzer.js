...
/**
 * Whether a blog needs Auto-fix (missing core SEO fields).
 */
export function blogNeedsAutoFix(blog = {}) {
  const title = String(blog.meta_title || '').trim();
  const desc = String(blog.meta_description || '').trim();
  const keywords = String(blog.meta_keywords || '').trim();
  const author = String(blog.author || '').trim();
  return !title || !desc || !keywords || !author;
}

/**
 * Whether a blog needs a meta Rewrite (weak / incomplete SERP copy).
 * Healthy meta → false so the Rewrite button can stay inactive.
 */
export function blogNeedsRewrite(blog = {}) {
  if (blogNeedsAutoFix(blog)) return true;

  const title = String(blog.meta_title || '').trim();
  const desc = String(blog.meta_description || '').trim();
  const h1 = String(blog.title || '').trim();

  if (title.length < 35 || title.length > 60) return true;
  if (desc.length < 120 || desc.length > 160) return true;
  if (h1 && title.toLowerCase() === h1.toLowerCase()) return true;
  // Soft CTA / commercial intent missing from description
  if (!/\b(call|remote|zamzam|support|fix|minutes?)\b/i.test(desc)) return true;

  return false;
}

/**
 * Summarize which bulk actions should be active.
 */
export function summarizeSeoNeeds(blogs = []) {
  let needsFix = 0;
  let needsRewrite = 0;
  for (const blog of blogs) {
    if (blogNeedsAutoFix(blog)) needsFix++;
    if (blogNeedsRewrite(blog)) needsRewrite++;
  }
  return {
    total: blogs.length,
    needsFix,
    needsRewrite,
    canAutoFix: needsFix > 0,
    canRewrite: needsRewrite > 0,
  };
}
...
export function buildBlogSeoPatch(blog, mode = 'empty') {
  const needsWork = mode === 'rewrite' ? blogNeedsRewrite(blog) : blogNeedsAutoFix(blog);
  ...
  // Only "changed"/writable when there is a real need
  const changed = needsWork && (mode === 'rewrite' || fieldChanged);

  return { patch, analysis, changed, needsWork };
}
