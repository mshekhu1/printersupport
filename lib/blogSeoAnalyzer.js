...
function buildMetaTitle({ title, brand, primaryKeyword }) {
  const base = stripMarkdown(title || primaryKeyword || 'Printer Fix')
    .replace(/\s+/g, ' ')
    .trim();
  let candidate = base;
  ...
  if (brand && !new RegExp(brand.label, 'i').test(candidate)) {
    candidate = `${brand.label} ${candidate}`;
  }
  ...
  if (candidate.length > 60) {
    candidate = clampSentence(candidate.replace(/\s*—\s*Fix Guide$/i, ''), 57);
  }
  return candidate;
}

function buildMetaDescription({ title, brand, primaryKeyword, plain, intents }) {
  const brandBit = brand ? `${brand.label} ` : '';
  const intentBit = intents[0] || primaryKeyword || 'printer issue';
  const hook = clampSentence(
    plain ||
      `${brandBit}${intentBit}: clear Windows steps that usually restore printing the same day.`,
    95
  );
  const cta = ' Stuck? Call ZamZam for remote US printer support.';
  let desc = `${hook}${cta}`;
  if (desc.length < 120) {
    desc = `Fix ${brandBit}${intentBit} with step-by-step checks for Windows 10/11.${cta}`;
  }
...
export function analyzeBlogSeo(input = {}) {
  ...
  const bodyMd = input.description || input.content || '';
  ...
  const suggestions = {
    ...
    meta_title: buildMetaTitle({ title, brand, primaryKeyword }),
