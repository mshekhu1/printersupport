...
const SITE_URL = (process.env.NEXT_PUBLIC_SITE_URL || process.env.SITE_URL || 'https://www.zamzamprint.com').replace(/\/+$/, '');
...
export function article({
  ...
  const schema = {
    ...
    description,
    author: {
      '@type': author === publisher ? 'Organization' : 'Person',
      name: author,
    },
    publisher: {
      '@type': 'Organization',
      ...
      logo: {
        '@type': 'ImageObject',
        url: `${SITE_URL}/side-view-employee-using-printer.jpg` // Fallback logo or specific logo if available
      },
      url: SITE_URL,
    },
    datePublished,
...
export function faqPage(faqs = []) {
  ...
  const mainEntity = faqs
    ...
    .map(faq => ({
      '@type': 'Question',
      name: stripMarkdown(faq.q || faq.question),
      acceptedAnswer: {
        '@type': 'Answer',
